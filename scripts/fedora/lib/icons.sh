#!/usr/bin/env bash
# Icon theme metadata for vm_rice Fedora (sourced, not executed)
set -euo pipefail
...
install_bibata_cursor() {
  # Fedora: try dnf first, then clone from upstream releases
  if dnf_pkg_installed bibata-cursor-theme; then
    ok "bibata-cursor-theme already installed (dnf)"
    return 0
  fi
  if run_as_root dnf install -y bibata-cursor-theme 2>/dev/null; then
    ok "bibata-cursor-theme installed via dnf"
    return 0
  fi
  log "Installing Bibata cursor from upstream release..."
  local tmp
  tmp="$(mktemp -d)"
  if curl -fsSL -o "${tmp}/bibata.tar.xz" \
    "https://github.com/ful1e5/Bibata_Cursor/releases/download/v2.0.6/Bibata-Modern-Classic.tar.xz"; then
    tar -xJf "${tmp}/bibata.tar.xz" -C "$ICON_DEST"
    ok "Bibata cursor installed from GitHub release"
  else
    warn "bibata-cursor-theme unavailable — install manually"
  fi
  rm -rf "$tmp"
}

install_qogir_cursor() {
  if dnf_pkg_installed qogir-cursor-theme; then
    ok "qogir-cursor-theme already installed (dnf)"
    return 0
  fi
  if run_as_root dnf install -y qogir-cursor-theme 2>/dev/null; then
    ok "qogir-cursor-theme installed via dnf"
    return 0
  fi
  log "Installing Qogir cursor from upstream..."
  local dest
  dest="$(clone_or_update "https://github.com/vinceliuice/Qogir-icon-theme.git" Qogir-cursors)"
  if [[ -x "${dest}/install.sh" ]]; then
    pushd "$dest" >/dev/null
    ./install.sh -c || warn "Qogir cursor install had issues"
    popd >/dev/null
    ok "Qogir cursor installed from source"
  else
    warn "qogir-cursor-theme unavailable — install manually"
  fi
}
